...
export async function sendNotificationEmail(input) {
  try {
    ...
    const confirmUrl = `${endPointFrontend}/verify-email/${
      input == null ? void 0 : input.accessToken
    }`;
    // const forgetPass = `${endpoint}/api/v1/user/authentication/reset/${input?.forgetPassToken}`
    const forgetPass = `${endPointFrontend}/lost-password/${
      input == null ? void 0 : input.forgetPassToken
    }`;
    ...
    let transporter = nodemailer.createTransport({
      ...
      secure: true,
      auth: {
        user: process.env.NODETAILER_EMAIL,
        pass: process.env.NODETAILER_PASSWORD,
      },
    });
    ...
    switch (input.emailData.emailType) {
      case "CONFIRMATION_EMAIL":
        var ref;
        const mailOptions1 = {
          from: `${input.emailData.senderAddress} <${process.env.NODETAILER_EMAIL}>`,
          to: input.userEmail,
        ...
        const info1 = await transporter.sendMail(mailOptions1).then((info) => {
      ...
      case "JOB_PUBLISHED":
        ...
        const info2 = await transporter.sendMail(mailOptions2).then((info) => {
          const userId = input == null ? void 0 : input.userId;
          const notificationInput = {
            user: input == null ? void 0 : input.userId,
            notification: [
      ...
      case "FORGET_PASSWORD":
        ...
        const info4 = await transporter.sendMail(mailOptions4).then((info) => {
          const userId = input == null ? void 0 : input.userId;
          const notificationInput = {
            user: input == null ? void 0 : input.userId,
            notification: [
      ...
      case "JOB_ALERT":
        ...
        const info5 = await transporter.sendMail(mailOptions5).then((info) => {
          const userId = input == null ? void 0 : input.userId;
          const notificationInput = {
            user: input == null ? void 0 : input.userId,
            notification: [
